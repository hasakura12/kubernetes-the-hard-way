...
function kube-apiserver::configure() {
  INTERNAL_IP=$(ip addr show enp0s8 | grep "inet " | awk '{print $2}' | cut -d / -f 1)
  echo $INTERNAL_IP
...
function kube-controller-manager::configure() {
...
function kube-scheduler::configure() {
...
# execute this on each master node
function main() {
  prerequisites::install
  etcd::configure
  kube-apiserver::configure
  kube-controller-manager::configure
